package jejuOseyo.vo;

import java.util.Date;

public class MateVO {
	private int mno;  //게시글 번호
	private String mtitle;  //게시글 제목
	private String mnick;  //게시글(회원) 닉네임_MemberVO 통일
	private int mhit;  //게시글 조회수
	private Date depart;  //여행 출발일
	private Date fin;  //여행 마지막일
	private String mplace;  //여행 장소
	private int mnum;  //참가인원
	private String mtext;  //게시글 내용
	
	public MateVO(int mno, String mtitle, String mnick, int mhit, Date depart, Date fin, String mplace, int mnum,
			String mtext) {
		super();
		this.mno = mno;
		this.mtitle = mtitle;
		this.mnick = mnick;
		this.mhit = mhit;
		this.depart = depart;
		this.fin = fin;
		this.mplace = mplace;
		this.mnum = mnum;
		this.mtext = mtext;
	}

	public int getMno() {
		return mno;
	}

	public void setMno(int mno) {
		this.mno = mno;
	}

	public String getMtitle() {
		return mtitle;
	}

	public void setMtitle(String mtitle) {
		this.mtitle = mtitle;
	}

	public String getMnick() {
		return mnick;
	}

	public void setMnick(String mnick) {
		this.mnick = mnick;
	}

	public int getMhit() {
		return mhit;
	}

	public void setMhit(int mhit) {
		this.mhit = mhit;
	}

	public Date getDepart() {
		return depart;
	}

	public void setDepart(Date depart) {
		this.depart = depart;
	}

	public Date getFin() {
		return fin;
	}

	public void setFin(Date fin) {
		this.fin = fin;
	}

	public String getMplace() {
		return mplace;
	}

	public void setMplace(String mplace) {
		this.mplace = mplace;
	}

	public int getMnum() {
		return mnum;
	}

	public void setMnum(int mnum) {
		this.mnum = mnum;
	}

	public String getMtext() {
		return mtext;
	}

	public void setMtext(String mtext) {
		this.mtext = mtext;
	}

	

}
